...
def test_generate_random_graph():
    ...
    assert (
        str(excinfo.value)
        == "Invalid random_type: Invalid. Please choose one of ER / WS / BA / KB / newman_WS"
    )
    ...
    assert (
        str(excinfo.value) == "Please provide valid input for number of nodes and edges"
    )
...
def test_spread_of_information():
    ...
    adopters_list_d = spread_of_information(
        G, timestamp, percentage_initial_adopt, type_initial_adopt, centrality_measure
    )
    ...
    adopters_list_p = spread_of_information(
        G, timestamp, percentage_initial_adopt, type_initial_adopt, centrality_measure
    )
    ...
    adopters_list_c = spread_of_information(
        G, timestamp, percentage_initial_adopt, type_initial_adopt, centrality_measure
    )
...
def test_make_diffusions_and_average():
    ...
    result = make_diffusions_and_average(
        test_graph,
        num_simulations=5,
        timestamp=5,
...
def test_run_simulation():
    ...
    ran_perc, cen_perc, mar_perc = run_simulation(
        G,
        num_simulations,
        time_steps,
