...
def main():
    ...
    G_fb = nx.read_edgelist(
        f"../data/{name}_edges.csv", delimiter=","
    ...
    # run the simulation
    ran_avg, cen_avg, mar_avg = run_simulation(
        G_fb,
        num_simulations,
        time_steps,
    ...
    for i in range(num_random_graphs):
        ...
        # run the simulation
        er_ran_perc, er_cen_perc, er_mar_perc = run_simulation(
            G_er,
            num_simulations,
            time_steps,
            percentage_initial_adopt,
        ...
        ws_ran_perc, ws_cen_perc, ws_mar_perc = run_simulation(
            G_ws,
            num_simulations,
            time_steps,
            percentage_initial_adopt,
        ...
        ba_ran_perc, ba_cen_perc, ba_mar_perc = run_simulation(
            G_ba,
            num_simulations,
            time_steps,
            percentage_initial_adopt,
        ...
        nw_ws_ran_perc, nw_ws_cen_perc, nw_ws_mar_perc = run_simulation(
            G_nw_ws,
            num_simulations,
            time_steps,
            percentage_initial_adopt,
        ...
        if i == 0:
            ...
            collected_results["newman_WS"] = [
                (nw_ws_ran_perc, nw_ws_cen_perc, nw_ws_mar_perc)
            ]
        else:
            ...
            collected_results["newman_WS"].append(
                (nw_ws_ran_perc, nw_ws_cen_perc, nw_ws_mar_perc)
            )
    ...
    plotting_structure(
        (
            averaged_results[name][0],
            averaged_results["ER"][0],
            averaged_results["WS"][0],
            averaged_results["BA"][0],
            averaged_results["newman_WS"][0],
        ),
        "random",
    )
    plotting_structure(
        (
            averaged_results[name][1],
            averaged_results["ER"][1],
            averaged_results["WS"][1],
            averaged_results["BA"][1],
            averaged_results["newman_WS"][1],
        ),
        "central",
    )
    plotting_structure(
        (
            averaged_results[name][2],
            averaged_results["ER"][2],
            averaged_results["WS"][2],
            averaged_results["BA"][2],
            averaged_results["newman_WS"][2],
        ),
        "marginal",
    )
